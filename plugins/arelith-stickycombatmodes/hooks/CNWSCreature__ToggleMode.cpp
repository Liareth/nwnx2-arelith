...
std::unique_ptr<FunctionHook> Hooks::CNWSCreature__ToggleMode::g_hookPtr;

void Hooks::CNWSCreature__ToggleMode::Hook()
{
    g_hookPtr = std::make_unique<FunctionHook>(
        "CNWSCreature::ToggleMode",
        Functions::CNWSCreature__ToggleMode,
        getFuncAddr(&CNWSCreature__ToggleMode__Hook));
}
...
void Hooks::CNWSCreature__ToggleMode__Hook(CNWSCreature* thisPtr, const uint8_t mode)
{
    ...
    g_hookPtr->callOriginal<CallingConvention::ThisCall, int32_t>(thisPtr, mode);
}