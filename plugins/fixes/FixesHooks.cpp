...
// Returns true if the var list contains an variable that starts with an underscore.
bool varListContainsNoStack(CNWObjectVarList* varList)
{
    if (varList == nullptr)
    {
        return false;
    ...
    for (unsigned int i = 0; i < varList->VarCount; i++)
    {
        CNWObjectVarListElement* var = &varList->VarList[i];

        if (var->sVarName.Text[0] == '_')
        {
            return true;
        }
    }
...
// returns true if stacking is not allowed for 2 items based on their variables, underscore variables prevent stacking, only if not identical named
bool varListCompareNoStack(CNWObjectVarList* varList1, CNWObjectVarList* varList2)
{
  if (varList1 == nullptr)
  {
    // checking for any underscore variables on item 2 is enough
    return varListContainsNoStack(varList2);
  }
  if (varList2 == nullptr)
  {
    // checking for any underscore variables on item 1 is enough
    return varListContainsNoStack(varList1);
  }
  ...
  // both items have variables, compare the underscore ones
  for (i = 0; i < varList1->VarCount; i++)
  {
      CNWObjectVarListElement* var = &varList1->VarList[i];

      if (var->sVarName.Text[0] == '_')
      {
        unique = 1;

        // check if item 2 doesn't have this underscore variable as well
        for (i2 = 0; i2 < varList2->VarCount; i2++)
        {
          CNWObjectVarListElement* var2 = &varList2->VarList[i2];

          if (var->sVarName.Text == var2->sVarName.Text)
          {
            // variable with same name found
            unique = 0;
            i2 = varList2->VarCount;
          }

        }

        // was the underscore variable unique?
        if (unique == 1)
        {
          // items shouldn't stack
          return true;
        }
        else
        {
  ...
  // we need to check the other way around as well, perhaps item2 has underscore variables as well
  for (i2 = 0; i2 < varList2->VarCount; i2++)
  {
      CNWObjectVarListElement* var2 = &varList2->VarList[i2];

      if (var2->sVarName.Text[0] == '_')
      {
        unique = 1;

        // check if item 1 doesn't have this underscore variable as well
        for (i = 0; i < varList1->VarCount; i++)
        {
          CNWObjectVarListElement* var = &varList1->VarList[i];
  ...
  // if no unique underscore variables were found, return false, so the items will stack
  return false;
...
int GetIsMergeableHookProc(void *pItem1, void *pItem2)
{
    ...
    if(fixes.bHooked && lastRet)
    {
        ...
        // Block the merge if either list contains nostack, otherwise allow it.
        return !varListContainsNoStack(pVarList1) && !varListContainsNoStack(pVarList2);
    }
    else
    {
        return lastRet;
